...
- **NLP text classification** (fine-tuned DistilBERT transformer model)
...
│   │   ├── email_classifier.py  # DistilBERT model service (singleton)
...
│
├── model/
│   └── saved_models/            # Local cached Hugging Face models
│
├── notebooks/
...
| `HF_MODEL_ID` | `cybersectony/phishing-email-detection-distilbert_v2.4.1` | HuggingFace model identifier |
...
|-----------|-------------|
| **Lifespan Manager** | `@asynccontextmanager` that loads the DistilBERT model on startup via `classifier.load_model()`. If loading fails, the API starts in degraded mode and returns 503 on analysis endpoints. |
...
2. lifespan() → classifier.load_model()
   ├── Downloads DistilBERT from HuggingFace Hub (first run)
...
| **Architecture** | DistilBERT (66M parameters, 6 transformer layers, 12 attention heads, 768 hidden size) |
...
| `load_model() → bool` | Downloads model + tokenizer from HuggingFace Hub, moves to device, sets eval mode. Returns `True` on success. |
...
| **HuggingFace ID** | `cybersectony/phishing-email-detection-distilbert_v2.4.1` |
...
These are **not installed** in the local `.venv`.
...
| Model Hosting | HuggingFace Hub (downloaded at startup) |
...
- Fine-tuned DistilBERT — V1 (98.63%), V2 (99.17%)
- Evaluated and tuned model performance
...
1. **Custom AI-generated phishing dataset** — 1,990 LLM-generated samples (first-of-its-kind)
